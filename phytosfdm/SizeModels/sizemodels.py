...
class SM:
    ...
    def initialconditions(self):
        ...
        elif self.Model == 'Imm_Zx2':
            return np.concatenate(([[self.Params['N0']], [self.Params['H0']], [self.Params['D0']], [self.Params['P0']],
                                    [self.Params['L_mean0']], [self.Params['L_var0']], [0] * 29, [self.Params['C0']], [0] * 3]), 0)
        else:
    ...
    def setup_params(self):
        ...
        default_parameters = {
            ...
            #<Zx2 parameters
            'muH': 1.35,               # Herbivorous Zooplankton maximum grazing rate (d^-1)
            'moH': 0.1,                # Herbivorous Zooplankton mortality
            'muC': 0.5,                # Carnivore maximum grazing rate (d^-1)
            'mpC': 1.2,                # Carnivore maximum processing rate (d^-1)
            'moC': 0.2,                # Carnivore mortality
            # Zx2>
            'Kp': 0.1,                 # P half-saturation constant (mmol*m^-3) ######
            'deltaZ': 0.31,            # P assimilation coefficient (-)
            # <Zx2
            'deltaH': 0.31,             # P assimilation coefficient (-)
            'deltaC': 0.01,             # H assimilation coefficient (-) #might be lower! interesting ratio!
            # Zx2 >
            ...
            #<Zx2
            'H0': 0.1,                 # Initial Herb. Zooplankton concentration (mmol*m^-3)
            'C0': 0.1,                # Initial Carnivore concentration (mmol*m^-3) ?!concentration?!
            #Zx2>
    ...
    def sizemodel_imm_Zx2(self, x, t):
        """
        This size based model variant is based on Acevedo-Trejos et al. (2015) in Sci. Rep.
        The immigration model includes a carnivore zooplankton type here.
        Z is split into H and C.
        ...
        # Initialization of state variables and dummy variables to store the biomass fluxes
        N = x[0]
        H = x[1]  # former Z -> Herbivorous Zooplankton
        D = x[2]
        P = x[3]
        L = x[4]
        V = x[5]
        C = x[35]  # Carnivorous Zooplankton
        dxdt = np.zeros(39)
        ...
        # state variables (maybe reorder)
        dxdt[0] = -P * (Gains + EN) + Mineralization + NMixing  # Nutrients
        #<Zx2
        dxdt[1] = HerbGrowth + EH - HerbMixing - CarnGrazing - HerbMortality  # Herb Zooplankton
        dxdt[35] = CarnGrowth - CarnMortality - CarnMixing  # Carnivore
        dxdt[2] = UnassimilatedProduction + P * OtherPMortalities + EHD - Mineralization - DetritusMixing + UnassimilatedCarnivory + HerbMortality + CarnMortality # Detritus
        #Zx2>
        dxdt[3] = P * (Gains - Losses + E) + Immigration  # Phytoplankton
        dxdt[4] = V*d1 + m3*0.5*d2 + Immigration/P*(self.Params['L_mean0']-L)  # Mean Size
        dxdt[5] = 0.5*(m4-1.)*V**2*d2 + Immigration/P*((self.Params['L_var0']-V) + (self.Params['L_mean0']-L)**2)  # Size Variance
        ...
        dxdt[8] = LightHarvesting  # Light Harvesting
        dxdt[9] = TemperatureDepGrowth  # Phytoplankton temperature dependency
        dxdt[10] = P * HerbGrazing  # Herbivore Grazing
        dxdt[11] = P * Sinking  # Phytoplankton Sinking
        dxdt[12] = P * OtherPMortalities  # Other P mortalities
        dxdt[13] = P * Mixing  # Phytoplankton Mixing
        dxdt[14] = HerbGrowth  # Herb Zooplankton Growth
        dxdt[15] = HerbMortality  # Herb Zooplankton Predator Mortality
        dxdt[16] = HerbMixing  # Herb Zooplankton Mixing
        #<Zx2
        dxdt[36] = CarnGrowth  # Carnivore Growth
        dxdt[37] = CarnMortality  # Carnivore Predator Mortality
        dxdt[38] = CarnMixing  # Carnivore Mixing
        #Zx2>
        dxdt[17] = UnassimilatedProduction  # Unassimilated Production
        dxdt[18] = Mineralization  # Mineralization
        dxdt[19] = DetritusMixing  # Detritus Mixing
        ...
        dxdt[22] = P*E  # P Higher order correction
        dxdt[23] = P*EN  # N Higher order correction
        dxdt[24] = EH  # Herbivore Grazing Higher order correction
        dxdt[25] = EHD  # Herbivore Unassimilated Production Higher order correction
        dxdt[26] = V*d1  # Changes in mean size
        dxdt[27] = Immigration/P*(self.Params['L_mean0']-L)  # Immigrating mean size
        dxdt[28] = 0.5*(m4-1.)*V**2*d2  # Changes in size variance
        dxdt[29] = Immigration/P*((self.Params['L_var0']-V) + (self.Params['L_mean0']-L)**2)  # Immigrating size variance
    ...
    def modelrun(self):
        ...
        try:
            if self.Model == "Imm":
                ...
                outarray = odeint(self.sizemodel_imm_Zx2, self.initcond, self.timedays)#,rtol=1.e-10, atol=1.e-12, hmax=1.0e16, hmin=1.0e-8)
                return outarray
            ...
            raise Exception("InputError:", "it is not a valid size model variant. Please specify one model variant "
                                           "from: Imm, Imm_Zx2, TraitDif, Fixvar, UnsustVar or FullModel.\n")
